...
class Linfun {
  constructor(coordArray) {
    ...
    if (checkedCoordArray.length < 1) {
      // Make sure there is always at least 1 coord
      const coord = [0, 0];
      Object.freeze(coord);
      checkedCoordArray.push(coord);
    }
  ...
  get last() { return this.array[this.length - 1]; }

  get tMin() { return this.first[0]; }

  get tMax() { return this.last[0]; }

  get tStep() { return this.tMax - this.tMin; }

  get vMin() { return this.array.reduce((acc, coord) => Math.min(acc, coord[1]), Infinity); }

  get vMax() { return this.array.reduce((acc, coord) => Math.max(acc, coord[1]), -Infinity); }

  get vStep() { return this.vMax - this.vMin; }
