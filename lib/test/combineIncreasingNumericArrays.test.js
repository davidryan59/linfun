import combineIncreasingNumericArrays from '../src/combineIncreasingNumericArrays';

test('test combine no arrays', () => {
  const arr = combineIncreasingNumericArrays();
  expect(arr).toEqual(
    [],
  );
});
...
test('test large time - values more than 1e300 get ignored', () => {
  const arr = combineIncreasingNumericArrays(
    [1, 1.2e300],
    [4, 1.1e300],
  );
