...
import evaluateTsOnArrayOfIncreasingTimes from '../src/evaluateTsOnArrayOfIncreasingTimes';
...
test('Simple test using whole numbers', () => {
  ...
  const resultArr = evaluateTsOnArrayOfIncreasingTimes(ts, arr);
  expect(resultArr).toEqual([5, 5, 1, -3, 40, 83, 126, 124, 122, 122]);
});

test('Complicated test using decimals and invalid time array values', () => {
  const ts = new Linfun([
    [-1.05, 30.452],
    [-1.04999, -19.2434],
    [1.04999, 12.342],
...
test('Test on bad time array', () => {
  const ts = new Linfun([[1, 2], [2, 4]]);
